                ...
                <tr v-for="game in games" :key="game.id">
                  ...
                  <td>
                    <div class="action-row">
                      <button class="action-btn" @click="openEdit(game)">
                      ...
                      <button class="action-btn action-btn--danger" @click="remove(game.id)">
...
const coverPreviewUrl = computed(() =>
  form.coverFile ? URL.createObjectURL(form.coverFile) : null
)
...
async function openEdit(game: Game) {
  Object.assign(form, {
    ...
    year: game.year ?? '',
    instagram_url: game.instagram_url ?? '',
    deposit_tokens: game.deposit_tokens ?? '',
    is_active: game.is_active,
    tag_ids: game.tags?.map((t) => t.id) ?? [],
    language_ids: game.languages?.map((l) => l.id) ?? [],
    coverFile: null,
  ...
  try {
    const detail = await useApi().get<{ data: Game & { images?: GameImage[] } }>(
      `/admin/games/${game.id}`
    )
    form.existingImages = detail.data.images ?? []
  } catch {
    /* images not critical — silently skip */
  }
}
...
async function save() {
  ...
  try {
    ...
    fields.forEach((f) => {
      if (form[f] !== '' && form[f] !== null) fd.append(f, String(form[f]))
    })
...
async function doExport() {
  ...
  try {
    ...
    alert((err as { message?: string }).message ?? t('common.error.export_failed'))
  } finally {
