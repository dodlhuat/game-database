                ...
                <tr v-for="event in events" :key="event.id">
                  ...
                  <td>
                    <div class="action-row">
                      <button class="action-btn" @click="openEdit(event)">{{ $t('admin.actions.edit') }}</button>
                      <button class="action-btn action-btn--danger" @click="remove(event.id)">{{ $t('admin.actions.delete') }}</button>
                    </div>
                  </td>
                </tr>
              </tbody>
      ...
      <div v-if="form.open" class="modal-overlay" @click.self="closeForm">
        <div class="dialog dialog--wide">
          <div class="dialog__header">
            <h3 class="dialog__title">{{ form.id ? $t('admin.events.edit') : $t('admin.events.add') }}</h3>
                      ...
                      <div class="file-item-header">
                        <div class="file-info">
                          ...
                          <div class="file-details">
                            <span class="file-name">{{ form.imageFile ? form.imageFile.name : $t('admin.events.form.image_current') }}</span>
                            <span class="file-size">{{ form.imageFile ? formatFileSize(form.imageFile.size) : '' }}</span>
                          </div>
                        </div>
...
function openCreate() {
  Object.assign(form, {
    open: true, id: null, title: '', date: '', time: '',
    is_all_day: false, description: '', imageFile: null, existingImageUrl: null, removeImage: false,
  })
  formError.value = ''
}
...
.form-grid {
  ...
  &__center { display: flex; align-items: flex-end; padding-bottom: 0.25rem; }
}
