...
.events-page { min-height: 100vh; display: flex; flex-direction: column; background: var(--background); }
...
.page-hero {
  ...
  &__body { position: relative; z-index: 1; max-width: 900px; margin: 0 auto; }
...
// ─── Content ──────────────────────────────────────────────────────
.events-content {
  flex: 1; padding: 1.5rem 1.25rem 4rem;
  &__inner { max-width: 900px; margin: 0 auto; }
}

.events-state { display: flex; justify-content: center; align-items: center; min-height: 300px; }
...
.dialog {
  ...
  &__title {
    ...
    letter-spacing: -0.02em; color: var(--primary-text);
  }
  ...
  &__close {
    ...
    &:hover { background: var(--background); color: var(--primary-text); }
  }
...
.modal-leave-to {
  opacity: 0;
  .dialog { opacity: 0; transform: translateY(8px) scale(0.98); }
}
...
.event-modal {
  ...
  &__time {
    ...
    font-size: 0.85rem; font-weight: 500; color: var(--secondary-text);
    .icon { font-size: 1rem; color: $amber; }
  }
...
.dialog--event {
  max-width: 540px;
  .dialog__body { overflow-y: auto; max-height: 70vh; }
}

// ─── Rich content reset ───────────────────────────────────────────
.rich-content :deep(h1),
.rich-content :deep(h2),
.rich-content :deep(h3) { margin: 0.75rem 0 0.35rem; font-weight: 700; }
.rich-content :deep(p)  { margin: 0 0 0.6rem; }
.rich-content :deep(ul),
.rich-content :deep(ol) { margin: 0 0 0.6rem 1.25rem; }
.rich-content :deep(a)  { color: $amber; }
</style>
