  ...
  <div class="legal-page">
    ...
    <section class="legal-hero">
      ...
      <div class="legal-hero__body">
        ...
        <h1 class="legal-hero__title">{{ $t('legal.cookies.title') }}</h1>
      </div>
    </section>
    ...
    <div class="legal-content">
      <div class="legal-content__inner">
        ...
        <!-- Unavailable -->
        <div v-else-if="!cookies" class="legal-empty">
          <span class="icon icon-article legal-empty__icon" aria-hidden="true" />
          <p class="legal-empty__text">{{ $t('legal.cookies.unavailable') }}</p>
        </div>

        <!-- Document -->
        <template v-else>
          <div class="legal-meta">
            <span class="legal-meta__version">v{{ cookies.version }}</span>
            <span class="legal-meta__sep" aria-hidden="true">·</span>
            <time class="legal-meta__date">{{ formatDate(cookies.published_at) }}</time>
          </div>
          <div class="legal-body">{{ cookies.content }}</div>
        </template>
      </div>
    </div>
  </div>
...
const cookies = ref<CookieVersion | null>(null)

onMounted(async () => {
  try {
    cookies.value = await api.get<CookieVersion>('/cookies')
  } catch {
    cookies.value = null
  } finally {
    loading.value = false
